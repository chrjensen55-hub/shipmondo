...
- Configurable pricing engine with percentage/fixed markup, minimum margin, minimum price, and rounding, applied on top of Shipmondo's real `/quotes/list` price where available. Where Shipmondo can't estimate a route (flagged `estimated: true` in quote metadata), pricing falls back to real per-carrier weight-class rate cards (`src/lib/carrierRates.ts`, sourced from each carrier's own published prices for Home vs. Service point/PakkeShop delivery) rather than a generic per-kg guess. The actual carrier cost is always re-confirmed as the authoritative purchase price once a shipment is booked.
- Home vs. Service point (pickup point) is a real choice in the wizard's Parcel step, driving both the weight-class options shown (each carrier publishes its own brackets and max weight) and which Shipmondo product gets booked (`service_point_available` on `/products`, `automatic_select_service_point` on `/shipments`).
...
- Domestic Home/Service point rate cards are now in place for DAO, GLS, PostNord, and Bring (`carrierRates.ts`). Still open: PostNord's international zone pricing and the Værdiforsendelse (declared-value insurance) add-on were deliberately deferred — domestic-only for this pass.
