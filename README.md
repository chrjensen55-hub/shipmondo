...
- Configurable pricing engine with percentage/fixed markup, minimum margin, minimum price, and rounding, applied on top of Shipmondo's real `/quotes/list` price where available. Where Shipmondo can't estimate a route (flagged `estimated: true` in quote metadata), pricing falls back to Pak & Send's own published price list (`src/lib/carrierRates.ts`) — domestic Collect/Home weight classes, and international zone pricing (Europe 1/2, Norway+Switzerland+Liechtenstein, rest of world, Faroe Islands, Greenland) keyed by destination country — applied the same regardless of which carrier is booked, rather than a generic per-kg guess. The actual carrier cost is always re-confirmed as the authoritative purchase price once a shipment is booked.
- Home vs. Service point (pickup point) is a real choice in the wizard's Parcel step. It drives which Shipmondo product gets booked (`service_point_available` on `/products`, `automatic_select_service_point` on `/shipments`); the weight-class picker itself is carrier-independent (Pak & Send's own price list applies the same everywhere).
...
- `quotes.ts` — picks one bookable product per carrier for a route, prices it via the real, non-committing `POST /quotes/list` endpoint where Shipmondo can estimate it, and falls back to an interim internal formula (clearly flagged `estimated: true`) for the carrier/route combinations it can't (e.g. DHL Express and UPS on several routes in this sandbox account — a real, documented API limitation, not a bug). Tags each quote with the real `product_code`/`service_codes` needed to book it, and with the product's own `customs_declaration_required` flag from `/products` (the `requiresCustoms` field in quote metadata) — this, not a country-list guess, is what determines whether the wizard shows the customs step.
- `shipments.ts` — builds a real `parties`/`parcels`/`customs` request from a `ShipmentDraft` and books it via `POST /shipments`. Customs is sent whenever the booked product's `customs_declaration_required` is true — this applies to Documents shipments too (sent with `export_reason: 'documents'`), not just Goods, matching Shipmondo's own customs rules. Booking fails with a clear message rather than sending fabricated customs data when a required commodity code is missing. Shipments can be cancelled with `PUT /shipments/{id}/cancel`.
...
- Pak & Send's real price list (domestic Collect/Home + international zone pricing) is now wired into `carrierRates.ts` and applied the same across all carriers. Still open: the Værdiforsendelse (declared-value insurance) add-on was deliberately deferred, and the Europe 1/Europe 2 country lists were transcribed from the price sheet's footnote — worth a spot check against a live non-EU booking.
- The wizard's Customs step (contents description, value, HS/commodity code) now appears whenever Shipmondo's own `customs_declaration_required` says the destination/product needs it, for Documents as well as Goods — validate this against a real non-EU sandbox booking (e.g. DK → US/GB) once real customs-required carriers are reachable.
