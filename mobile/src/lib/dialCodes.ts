import { countries } from './countries'

// ITU calling codes keyed by the same ISO 3166-1 alpha-2 codes as countries.ts, so the phone
// field's country picker can reuse that list's names instead of keeping a second copy in sync.
const DIAL: Record<string, string> = {
  AF: '+93', AL: '+355', DZ: '+213', AD: '+376', AO: '+244', AG: '+1268', AR: '+54', AM: '+374',
  AU: '+61', AT: '+43', AZ: '+994', BS: '+1242', BH: '+973', BD: '+880', BB: '+1246', BY: '+375',
  BE: '+32', BZ: '+501', BJ: '+229', BT: '+975', BO: '+591', BA: '+387', BW: '+267', BR: '+55',
  BN: '+673', BG: '+359', BF: '+226', BI: '+257', KH: '+855', CM: '+237', CA: '+1', CV: '+238',
  CF: '+236', TD: '+235', CL: '+56', CN: '+86', CO: '+57', KM: '+269', CG: '+242', CD: '+243',
  CR: '+506', HR: '+385', CU: '+53', CY: '+357', CZ: '+420', DK: '+45', DJ: '+253', DM: '+1767',
  DO: '+1809', EC: '+593', EG: '+20', SV: '+503', GQ: '+240', ER: '+291', EE: '+372', SZ: '+268',
  ET: '+251', FJ: '+679', FI: '+358', FR: '+33', GA: '+241', GM: '+220', GE: '+995', DE: '+49',
  GH: '+233', GR: '+30', GD: '+1473', GT: '+502', GN: '+224', GW: '+245', GY: '+592', HT: '+509',
  HN: '+504', HK: '+852', HU: '+36', IS: '+354', IN: '+91', ID: '+62', IR: '+98', IQ: '+964',
  IE: '+353', IL: '+972', IT: '+39', CI: '+225', JM: '+1876', JP: '+81', JO: '+962', KZ: '+7',
  KE: '+254', KI: '+686', KW: '+965', KG: '+996', LA: '+856', LV: '+371', LB: '+961', LS: '+266',
  LR: '+231', LY: '+218', LI: '+423', LT: '+370', LU: '+352', MO: '+853', MG: '+261', MW: '+265',
  MY: '+60', MV: '+960', ML: '+223', MT: '+356', MR: '+222', MU: '+230', MX: '+52', MD: '+373',
  MC: '+377', MN: '+976', ME: '+382', MA: '+212', MZ: '+258', MM: '+95', NA: '+264', NP: '+977',
  NL: '+31', NZ: '+64', NI: '+505', NE: '+227', NG: '+234', MK: '+389', NO: '+47', OM: '+968',
  PK: '+92', PA: '+507', PG: '+675', PY: '+595', PE: '+51', PH: '+63', PL: '+48', PT: '+351',
  QA: '+974', RO: '+40', RU: '+7', RW: '+250', KN: '+1869', LC: '+1758', VC: '+1784', WS: '+685',
  SM: '+378', SA: '+966', SN: '+221', RS: '+381', SC: '+248', SL: '+232', SG: '+65', SK: '+421',
  SI: '+386', SB: '+677', SO: '+252', ZA: '+27', KR: '+82', SS: '+211', ES: '+34', LK: '+94',
  SD: '+249', SR: '+597', SE: '+46', CH: '+41', SY: '+963', TW: '+886', TJ: '+992', TZ: '+255',
  TH: '+66', TL: '+670', TG: '+228', TO: '+676', TT: '+1868', TN: '+216', TR: '+90', TM: '+993',
  TV: '+688', UG: '+256', UA: '+380', AE: '+971', GB: '+44', US: '+1', UY: '+598', UZ: '+998',
  VU: '+678', VA: '+379', VE: '+58', VN: '+84', YE: '+967', ZM: '+260', ZW: '+263',
}

export type DialCode = { code: string; name: string; dial: string }

export const dialCodes: DialCode[] = countries
  .filter((c) => DIAL[c.code])
  .map((c) => ({ code: c.code, name: c.name, dial: DIAL[c.code] }))

export const DEFAULT_DIAL_CODE = 'DK'

// Longest-prefix match against known dial codes so "+1868…" resolves to Trinidad rather than the
// shared NANP "+1" — falls back to Denmark with the whole value treated as the local number when
// nothing matches, which also covers old phone numbers saved before this picker existed.
export function splitPhone(phone: string): { countryCode: string; local: string } {
  const trimmed = phone.trim()
  if (!trimmed.startsWith('+')) return { countryCode: DEFAULT_DIAL_CODE, local: trimmed }
  const sorted = [...dialCodes].sort((a, b) => b.dial.length - a.dial.length)
  for (const entry of sorted) {
    if (trimmed === entry.dial || trimmed.startsWith(`${entry.dial} `) || trimmed.startsWith(entry.dial)) {
      return { countryCode: entry.code, local: trimmed.slice(entry.dial.length).trim() }
    }
  }
  return { countryCode: DEFAULT_DIAL_CODE, local: trimmed }
}

export function joinPhone(countryCode: string, local: string): string {
  const entry = dialCodes.find((d) => d.code === countryCode)
  const dial = entry?.dial ?? '+45'
  const digits = local.trim()
  return digits ? `${dial} ${digits}` : ''
}
